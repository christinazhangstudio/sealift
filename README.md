...
when a query is submitted, the Go backend asks local Ollama to turn the text into a vector. importantly, only the new question (`q`) is embedded — conversation history is kept separate so it doesn't pollute vector similarity scores.
